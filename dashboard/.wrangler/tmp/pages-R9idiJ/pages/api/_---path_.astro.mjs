...
app.get("/search", async (c) => {
  ...
  const domainResults = await db.select().from(domains).where(like(domains.domain, `%${query}%`)).limit(10);
  const ruleResults = await db.select().from(rules).where(or(like(rules.name, `%${query}%`), like(rules.category, `%${query}%`))).limit(10);
  return c.json({
...
app.get("/status", async (c) => {
  ...
  const [analyticsRes, healthRes] = await Promise.all([fetch("https://api.cloudflare.com/client/v4/graphql", {
    method: "POST",
    headers,
    body: JSON.stringify({
      query: gqlQuery
    })
  }).then((r) => r.json()).catch(() => null), fetch("https://gins-rules-scanner.ichimarugin728.workers.dev/classify?domain=cloudflare.com", {
  ...
  const acct = analyticsRes?.data?.viewer?.accounts?.[0];
...
const ALL = (context) => app.fetch(context.request, context.locals.runtime.env);
