...
export default function GinsIcons({ initialIcons }: { initialIcons: Icon[] }) {
  ...
  const filteredIcons = useMemo(() => {
    return initialIcons.filter((icon) => {
      const matchSearch = icon.name
        .toLowerCase()
        .includes(search.toLowerCase());
      const matchSource =
        activeSource === "All" || icon.source === activeSource;
      ...
      <div class="glass-panel p-6! flex flex-col gap-6">
          ...
          <div class="flex items-center gap-4">
            ...
            <div>
              ...
              <p class="text-[10px] text-gray-500 font-bold uppercase tracking-widest mt-0.5">
                Automated Multi-Source Library
              </p>
            </div>
          </div>
        ...
        <div class="flex flex-col sm:flex-row items-center gap-4 pt-6 border-t border-white/5">
          <div class="flex-1 flex items-center gap-3 w-full bg-black/20 border border-white/5 rounded-2xl px-4 py-2.5 group hover:border-brand-primary/30 transition-colors">
            ...
            <code class="text-[10px] font-mono text-gray-400 select-all truncate">
              https://rules.ichimarugin728.dev/Gins-Icons.json
            </code>
          </div>
          <button
            type="button"
          ...
          >
            ...
            "https://rules.ichimarugin728.dev/Gins-Icons.json" ? (
              ...
              <div class="i-ph-copy-bold"></div>
            )}
            {copiedUrl === "https://rules.ichimarugin728.dev/Gins-Icons.json"
              ? "COPIED"
              : "COPY JSON URL"}
          </button>
        </div>
      ...
      <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
        {filteredIcons.map((icon) => (
          <button
            ...
            class="glass-panel group cursor-pointer hover:border-brand-primary/40 hover:bg-white/[0.05] transition-all duration-300 p-4 flex flex-col items-center justify-center gap-4 relative overflow-hidden active:scale-95 text-left"
          >
            {/* Glossy Overlay */}
            ...
            <div class="absolute top-3 right-3 flex items-center justify-center w-6 h-6 rounded-lg bg-white/5 border border-white/5 group-hover:border-brand-primary/30 group-hover:bg-brand-primary/10 transition-all opacity-40 group-hover:opacity-100">
              <div
                class={`text-[10px] ${copiedUrl === icon.url ? "i-ph-check-bold text-green-400" : "i-ph-copy-simple-bold text-brand-primary"}`}
              ></div>
            </div>

            {/* Copy Feedback Overlay (Center) */}
            <div
              class={`absolute inset-0 flex items-center justify-center bg-brand-primary/90 transition-all duration-300 ${copiedUrl === icon.url ? "opacity-100" : "opacity-0 pointer-events-none"}`}
            >
