...
struct GinsRulesSyncer: AsyncParsableCommand {
    ...
    func runSync(root: URL) async throws {
        ...
        let results = try await withThrowingTaskGroup(of: (String, String, [String]).self) { group in
            for src in sources where src.enabled {
                group.addTask {
                    ...
                    guard let content = String(data: data, encoding: .utf8) else { return (src.category, src.target, []) }
        ...
        for (cat, targets) in results {
            ...
            for (name, rules) in targets {
                ...
                try content.write(to: outDir.appending(path: "\(name).txt"), atomically: true, encoding: .utf8)
    ...
    func runIcons(root: URL) async throws {
        ...
        let allIcons = try await withThrowingTaskGroup(of: [NormalizedIcon].self) { group in
            for src in sources where src.enabled {
                group.addTask {
                    ...
                    let (data, _) = try await URLSession.shared.data(from: url)
                    // Simplified parsing for now, assuming raw JSON
                    if let raw = try? JSONDecoder().decode([RawIcon].self, from: data) {
                        return raw.map { NormalizedIcon(name: $0.name ?? $0.tag ?? "icon", url: $0.url ?? "", source: src.name, theme: src.theme) }
                    }
                    return []
                }
            }
        ...
        let finalData = try encoder.encode(allIcons.sorted(by: { $0.name < $1.name }))
        try finalData.write(to: outPath)
        try? FileManager.default.createDirectory(at: dashboardPath.deletingLastPathComponent(), withIntermediateDirectories: true)
    ...
    func processRules(content: String) -> [String] {
        content.components(separatedBy: .newlines).compactMap { line in
            ...
            if parts.count >= 2 {
                ...
                let value = parts[1].split(separator: "//")[0].trimmingCharacters(in: CharacterSet(charactersIn: "'\" "))
                switch type {
