...
async function handleFeed(path: string, url: URL, env: Env): Promise<Response> {
  ...
  try {
    ...
    if (!manifestResp.ok) return new Response('Manifest not found', { status: 404 });

    const files: string[] = await manifestResp.json();
    const results = await Promise.all(
      files.map((f) => fetch(`${baseUrl}/${f}`).then((r) => r.text())),
    );

    let merged = results.join('\n');

    // Deduplicate text formats
    if (ext === 'list') {
      const lines = merged
        .split('\n')
        .map((l) => l.trim())
        .filter((l) => l && !l.startsWith('#'));
      merged = [...new Set(lines)].join('\n');
    }
